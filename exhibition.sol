...
contract Exhibition {
    ...
    function registration(string _name, string _phone, string _email) public payable returns(string, string, string){
    ...
    function random() private view returns (uint) {
        return uint(keccak256(abi.encodePacked(block.difficulty, now, participantList)));
    }

    function getting(uint r) public pure returns(uint) {
        return r;
    }
